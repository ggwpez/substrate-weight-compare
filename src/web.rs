...
async fn compare(commits: web::Path<(String, String, Option<String>)>) -> impl Responder {
	...
	let (old, new, thresh) = (commits.0.trim(), commits.1.trim(), commits.2.clone().unwrap_or_else(|| "30".into()));
	...
	let mut output =
		String::from_str("<table><tr><th>Extrinsic</th><th>Old [ns]</th><th>New [ns]</th><th>Diff [%]</th></tr>")
			.unwrap();
	...
	HttpResponse::Ok()
		...
		.body(format!("Compared Polkadot {old} (old) to {new} (new) with {thresh}% threshold:</br>{output}"))
}
...
async fn main() -> std::io::Result<()> {
	...
	HttpServer::new(|| {
		App::new().wrap(middleware::Compress::default()).service(compare).service(compare_index).service(root)
	})
	.bind(endpoint)?
	.run()
	.await
