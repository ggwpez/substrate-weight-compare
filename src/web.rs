...
async fn compare(commits: web::Path<(String, String, Option<String>)>) -> impl Responder {
    ...
    let (old, new, thresh) = (commits.0.trim(), commits.1.trim(), commits.2.clone().unwrap_or("30".into()));
    ...
    let mut output = String::from_str("<table><tr><th>Extrinsic</th><th>Old [ns]</th><th>New [ns]</th><th>Diff [%]</th></tr>").unwrap();
...
async fn compare_index() -> HttpResponse {
    HttpResponse::Ok().content_type("text/html; charset=utf-8").body("Help:</br>&nbsp;&nbsp;<a href='/compare/20467ccea1ae3bc89362d3980fde9383ce334789/master/30'>Example</a>")
}
