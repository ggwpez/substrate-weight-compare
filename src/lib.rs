	...
	for file in files {
		...
		if !news.contains_key(&file) {
			...
			for extrinsic in olds.get(&file).unwrap() {
				diff.get_mut(&file).unwrap().insert(extrinsic.0.clone(), ExtrinsicChange::Removed);
			}
			continue;
		...
		for extrinsic in extrinsics {
			...
			if p == 0.0 {
				...
				diff.get_mut(&file).unwrap().insert(extrinsic.clone(), ExtrinsicChange::Change(old_w, new_w, p));
			}
		}
	}
...
fn parse_files(paths: &Vec<PathBuf>, blacklist: &Vec<String>) -> Result<Map<String, Map<String, WeightNs>>, String> {
	let mut map = Map::new();
...
fn handle_method(m: &ImplItemMethod) -> (String, WeightNs) {
	...
	assert_eq!(m.block.stmts.len(), 1, "There must be only one statement per weight function");
...
fn lit_to_weight(lit: &Lit) -> WeightNs {
	match lit {
		Lit::Int(i) => i.base10_digits().parse::<u64>().unwrap().checked_div(1000).unwrap(),
		_ => unreachable!(),
	}
}
